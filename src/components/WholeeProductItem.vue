<template>
  <div class="container-fluid mt-4">
    <div>
      <h1 class="h1">Wholee Product Quick View</h1>
    </div>
    <div>
      ...
      <div class="table table-striped product-all">
        <div v-for="product in products.rows" :key="product.id" class="product-item">
          <a class="product-item-link" target="_blank"
            ...
            <a class="product-item-link-short" target="_blank" :href="`https://www.wholeeshopping.com/product/${product.id}`"
          ...
          <p class="is-active">
            ...
            <span class="mark">{{ product.orders }}</span>
          </p>
          <p class="is-active">
  ...
  methods: {
    ...
    async fetchAllProducts() {
      ...
      for (let i = 0; i < this.productIdList.length; i++) {
        ...
        product.body.categoryNames = product.body.categoryNames.reverse();
        console.log(product.body.categoryNames);
        this.products.rows.push(product.body);
      }
    },
  },
};
